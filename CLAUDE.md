...
### Claude Code使用時の注意事項
1. **必須**: プル後は必ずCLAUDE.mdを最初に読む
2. **ブランチ**: featureブランチで作業、mainは安定版のみ
3. **コミット**: Claude Code標準形式を使用
4. **環境**: `.env.example`をベースに環境構築
5. **テスト**: 機能追加時は動作確認必須

### 開発分担（推奨）
- **Frontend重点**: UI/UX改善、新コンポーネント
- **Backend重点**: API拡張、データベース、認証
- **機能重点**: 特定機能（地図、クイズ、スコア管理）
